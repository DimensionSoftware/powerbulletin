...
require.define("/app/mutants.ls",function(require,module,exports,__dirname,__filename,process,global){(function(){
  ...
  this.forum = {
    ...
    onInitial: function(window, next){
      return window.hasMutatedForum = true;
    },
...
require.define("/app/entry.ls",function(require,module,exports,__dirname,__filename,process,global){(function(){
  var $w, $d, sep, showLoginDialog, login, requireLogin, addPostDialog, addPost, appendReplyUi, switchAndFocus;
  $w = $(window);
  $d = $(document);
  ...
  switchAndFocus = function(e, remove, add, focusOn){
    $(e).removeClass(remove).addClass(add);
    return setTimeout(function(){
      return $(focusOn).focus();
    }, 100);
  };
  $d.on('click', '.onclick-show-login', function(){
    return switchAndFocus('.fancybox-wrap', 'on-forgot', 'on-login', '#auth input[name=username]');
  });
  $d.on('click', '.onclick-show-forgot', function(){
    return switchAndFocus('.fancybox-wrap', 'on-error', 'on-forgot', '#auth input[name=email]');
  });
